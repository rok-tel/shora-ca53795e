...
// Mock data for articles - would be replaced with actual API calls
...
export const createArticle = async (article: Omit<Article, 'id'>): Promise<Article> => {
  // This would be a real API call in production
  return new Promise((resolve) => {
    const newId = (mockArticles.length + 1).toString();
    const newArticle = { ...article, id: newId };
    mockArticles.push(newArticle);
    setTimeout(() => resolve(newArticle), 300);
  });
};
...
export const updateArticle = async (id: string, updates: Partial<Article>): Promise<Article> => {
  ...
  return new Promise((resolve, reject) => {
    ...
    setTimeout(() => resolve(mockArticles[index]), 300);
  });
};
